...
std::vector<std::string> tags(MAX_ENTITIES);
...
Entity create_icon_entity(std::string tag, Vector2 pos, Vector2 size,
		const char* imgpath, int padding, U64 groupid)
{
	U64 comp_mask = 0;
	U64 id = entityCount++;
...
void render_icons()
{
	for (int i = 0; i < entityCount; ++i) {
		...
		DrawTexturePro(images[e.id].texture,
				...
				Rectangle { positions[e.id].x, positions[e.id].y, sizes[e.id].width,
						sizes[e.id].height },
...
int main()
{
	...
	float y = 20;
	Entity bin = create_icon_entity("deleteBtn", Vector2 { x, y },
			Vector2 { iconSize, iconSize }, "assets/bin.png", iconPad, 0);
	Entity selection = create_icon_entity("selectionBtn", Vector2 { x, y },
			Vector2 { iconSize, iconSize }, "assets/selection.png", iconPad, 0);
	Entity pen = create_icon_entity("penBtn", Vector2 { x, y },
			Vector2 { iconSize, iconSize }, "assets/pen.png", iconPad, 0);
	Entity rect = create_icon_entity("rectangleBtn", Vector2 { x, y },
			Vector2 { iconSize, iconSize }, "assets/rectangle.png", iconPad, 0);
	Entity brush = create_icon_entity("brushBtn", Vector2 { x, y },
			Vector2 { iconSize, iconSize }, "assets/brush.png", iconPad, 0);
	Entity circle = create_icon_entity("circleBtn", Vector2 { x, y },
			Vector2 { iconSize, iconSize }, "assets/circle.png", iconPad, 0);
	Entity images = create_icon_entity("imagesBtn", Vector2 { x, y },
			Vector2 { iconSize, iconSize }, "assets/images.png", iconPad, 0);
