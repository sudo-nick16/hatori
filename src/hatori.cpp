	...
	for (int i = 0; i < rect_lines.size(); ++i) {
		...
		if (rect.z == coll_top_most_z) {
			if (mode == Draw_Mode::SELECTION_MODE) {
				...
				} else if (selected_rect != -1 && IsMouseButtonDown(MOUSE_BUTTON_LEFT)
						&& is_cursor_moving()) {
		...
		if (texts[i].z == text_top_most_z) {
			if (mode == Draw_Mode::SELECTION_MODE) {
				if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
					...
					prev_cursor_x = cursor_x;
					prev_cursor_y = cursor_y;
				} else if (selected_text != -1 && IsMouseButtonDown(MOUSE_BUTTON_LEFT)
						&& is_cursor_moving()) {
					...
					DrawRectangleLines(texts[i].x - 10, texts[i].y,
							MeasureText(texts[i].text.c_str(), 40) + 20, 40, WHITE);
				} else {
					DrawRectangleLines(texts[i].x - 10, texts[i].y,
...
Hatori_Icon create_icon_inside_con(
		Texture2D* text, Rectangle* rect, float side, float pad, int index)
{
	return Hatori_Icon {
		.texture = text,
		.parent = rect,
...
void draw_icon(Hatori_Icon icon)
{
	...
	DrawTexturePro(*icon.texture,
			Rectangle {
					0, 0, (float)icon.texture->width, (float)icon.texture->height },
...
int main()
{
	...
	Rectangle icon_container = Rectangle { GetScreenWidth() / 2.0f
				- ((ICON_SIDE + ICON_PAD) * N_ICONS) / 2,
		10, (ICON_SIDE + ICON_PAD) * N_ICONS, ICON_SIDE + ICON_PAD };

	Hatori_Icon bin_icon = create_icon_inside_con(
			&bin_txt, &icon_container, ICON_SIDE, ICON_PAD, 0);
	Hatori_Icon pointer_icon = create_icon_inside_con(
			&pointer_txt, &icon_container, ICON_SIDE, ICON_PAD, 1);
	Hatori_Icon rect_icon = create_icon_inside_con(
			&rect_txt, &icon_container, ICON_SIDE, ICON_PAD, 2);
	Hatori_Icon pen_icon = create_icon_inside_con(
			&pen_txt, &icon_container, ICON_SIDE, ICON_PAD, 3);
	Hatori_Icon text_icon = create_icon_inside_con(
			&text_txt, &icon_container, ICON_SIDE, ICON_PAD, 4);
	Hatori_Icon image_icon = create_icon_inside_con(
			&image_txt, &icon_container, ICON_SIDE, ICON_PAD, 5);
		...
		if (selected_text == -1) {
			for (int i = texts.size() - 1; i >= 0; --i) {
				if (CheckCollisionPointRec(pos,
								to_true_rect(Rectangle { texts[i].x, texts[i].y,
			...
			if (left_mouse_down) {
				if (mode == Draw_Mode::SELECTION_MODE && !selected_rect) {
					DrawRectangleLines(prev_cursor_x, prev_cursor_y,
							abs(cursor_x - prev_cursor_x), abs(cursor_y - prev_cursor_y),
							WHITE);
